...
│              │    │              │    │  (1,0,0)     │
└──────────────┘    └──────────────┘    └──────────────┘
  x²+y²+z²=1       x²/4+(2y)²+z²=1   (x-1)²/4+(2y)²+z²=1
```

**对应的数学方程演进：**

1. **标准球**（局部坐标）：
   $$\mathbf{u}^T \mathbf{u} = u_x^2 + u_y^2 + u_z^2 = 1$$
...
将上述方程重写为 $(\mathbf{x} - \boldsymbol{\mu})^T A (\mathbf{x} - \boldsymbol{\mu}) = 1$：

**步骤分解：**
$$\left(\frac{x-1}{2}\right)^2 + (2y)^2 + z^2 = 1$$
$$= \frac{(x-1)^2}{4} + 4y^2 + z^2 = 1$$
$$= \begin{bmatrix} x-1 & y & z \end{bmatrix} \begin{bmatrix} 1/4 & 0 & 0 \\ 0 & 4 & 0 \\ 0 & 0 & 1 \end{bmatrix} \begin{bmatrix} x-1 \\ y \\ z \end{bmatrix} = 1$$

**最终形式：**
$$(\mathbf{x} - \boldsymbol{\mu})^T A (\mathbf{x} - \boldsymbol{\mu}) = 1$$

其中：
- $\boldsymbol{\mu} = [1, 0, 0]^T$（中心位置）
- $A = \begin{bmatrix} 1/4 & 0 & 0 \\ 0 & 4 & 0 \\ 0 & 0 & 1 \end{bmatrix}$（形状矩阵，即 $\Sigma^{-1}$）

**对应标准差对照表：**

| 方向 | 系数 $a_i$ | 半轴长 = $1/\sqrt{a_i}$ | 标准差 $\sigma_i$ | 直观描述 |
|------|-----------|------------------------|------------------|----------|
| x    | $0.25$     | $2$                    | $\sigma_x = 2$   | "胖"（拉长） |
| y    | $4$       | $0.5$                  | $\sigma_y = 0.5$ | "瘦"（压扁） |
| z    | $1$       | $1$                    | $\sigma_z = 1$   | 正常 |
...
在 3DGS 中，每个高斯都由以下参数定义：

```python
# 伪代码示例
gaussian = {
    'position': [1.0, 0.0, 0.0],     # μ (中心)
    'scale': [2.0, 0.5, 1.0],        # σ_x, σ_y, σ_z (标准差)
    'rotation': R,                    # 旋转矩阵（可选，此处为 I）
}

# 协方差矩阵 Σ = R S R^T，其中 S = diag(σ²)
Sigma = [[4.0,   0,   0],         # σ_x²=4, σ_y²=0.25, σ_z²=1
         [0,   0.25,  0],
         [0,    0,   1]]

# 二次型矩阵 A = Σ^(-1)（注意是逆！）
A = Sigma.inverse() 
  = [[0.25, 0,   0],   # 对应 diag(1/4, 4, 1)
     [0,    4,   0],
     [0,    0,   1]]
```

**关键洞察：**
- 3DGS 的每个高斯都是一个**椭球等值面**：$(\mathbf{x} - \boldsymbol{\mu})^T \Sigma^{-1} (\mathbf{x} - \boldsymbol{\mu}) = 1$
- $\sigma$ 越大 → $A$ 的对角元素越小 → 该方向越"胖"
- 这就是为什么看到 $d^T \Sigma^{-1} d$ 时，要理解成"**在变形空间里测量距离**"
