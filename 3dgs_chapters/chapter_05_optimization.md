...
**本章核心问题**：第 4 章已经解释了为什么渲染链大部分可以反向传播。但“梯度能流回去”不等于“模型会学对”。真正的问题是：
...
如果你用更贴近实现的内部参数化，也可以写成：

```text
Theta = {mu_i, scale_i, rotation_i, opacity_i, sh_i}_i
```

这里 `sh_i` 可以理解成颜色或球谐外观参数。

给定一个训练视角 `cam_k`，渲染器会生成预测图像：
...
```text
Theta* = argmin_Theta E_k [ L_img(render(Theta, cam_k), I_gt^k) ]
```
...
这和 [../linear_algebra_engineering_perspective.md](../linear_algebra_engineering_perspective.md) 的主线其实是同一件事。
