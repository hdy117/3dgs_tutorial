...
<details>
<summary>提示</summary>
- 每步都 densify → 高斯数量爆炸式增长
- 梯度需要时间稳定后再决定是否增删
- 常见做法：每 100 步检查一次，前 K 步跳过 densify
</details>
...
2. **梯度需要稳定期**:
   - 初期训练：参数变化大，density 判断不准
   - 后期训练：loss 收敛后 densify 才更有意义
...
3. **常见策略** (3DGS 原论文):
   ...
   for step in range(max_steps):
       ...
       if step % 100 == 0 and step > 1000:  # 每 100 步，前 1000 步跳过
           densify_and_prune()
   ```
...
经验值：**每 100 步**检查一次，前 1000-5000 步跳过 densify。
</details>

### Q2: "L1 loss 和 SSIM loss 的权重比例怎么调？"
...
$$L_\text{img} = (1 - \lambda) \cdot L_1 + \lambda \cdot (1 - \text{SSIM})$$

| λ 取值 | 现象 |
|-------|------|
| **λ = 0.0** (纯 L1) | 颜色准确，但边缘/纹理容易糊 |
| **λ = 0.2** (默认) | 平衡：L1 保颜色，SSIM 保结构 |
| **λ = 0.8** (SSIM 主导) | 结构清晰但颜色可能偏（尤其高光区域）|

**调参经验**:
- **静态场景**: λ ∈ [0.15, 0.25] 通常足够
- **高动态范围**: 适当降低 L1 权重，让 SSIM 多贡献一点
- **低纹理区域**: 提高 L1 权重，避免 SSIM 误判

**核心原则**: L1 是"主粮"(保证基本准确)，SSIM 是"调味剂"(提升结构质量)。
</details>

### Q3: "prune 的触发条件是什么？"

为什么透明度阈值通常设在 0.005 左右？
...
<details>
<summary>答案</summary>
**Prune 的本质**: 删除"无用的高斯"，节省资源。

触发条件（3DGS 原论文）:
```python
if opacity < threshold:  # 通常 0.005
    remove_gaussian(i)
```

**为什么是 0.005?**

- **太小** (如 0.001): 保留太多透明高斯 → 内存浪费
- **太大** (如 0.01): 可能误删边缘区域的半透明部分

0.005 的经验值来源：
- 在典型分辨率下，alpha < 0.005 的高斯对最终图像贡献 < 0.5%
- 保留它们带来的收益 < 删除它们的计算节省

**注意**: Prune 不是独立操作，而是和 densify/split 配合的"容量管理":
- **Densify/Split**: 哪里不够 → 增加高斯
- **Prune**: 哪里的没用 → 删除高斯
...
1. 训练循环的四步是什么？为什么必须按这个顺序？
2. densify/split/prune 各自在什么条件下触发？
3. 为什么前 1000-5000 步通常跳过 densification？
4. L1 和 SSIM 权重的物理意义分别是什么？
5. 如何判断训练已经"收敛"(可以停止 densify)？
...
下一章 [chapter_08_inference_optimization.md](chapter_08_inference_optimization.md) 会解决这个工程问题:

- Tile-based rendering 为什么比朴素遍历快？
- Level of Detail (LOD) 怎么用？
- 实际部署时的性能优化策略## 十六、本章你真正应该能自己重建的几个问题
