...
如果某个方向 $\boldsymbol{\Sigma}$ 很宽（标准差大），那么在这个方向上移动相同的距离，二次型的增长会**比较小**（代价小）。反之，如果某个方向很窄，同样的偏移会被认为"很远"。
...
**工程价值**:
$$\text{复杂度从 } O(HW \cdot N) \to O(HW \cdot k) \quad (k \ll N)$$

这就是为什么 3DGS 能实时渲染——不是算法快，而是**数学特性让计算量天然减少**。
...
这是从**问题驱动的第一性原理**推导出的必然结果：

1. **$\boldsymbol{\mu} \in \mathbb{R}^3$ (中心位置)**
   - Axiom: 物体在空间中有确定位置
   - 不可约的最小表示

2. **$\boldsymbol{\Sigma} \in \mathbb{S}^3_+$ (协方差矩阵，3D 椭球形状)**
   - Problem: 点没有体积 → 需要"软"的局部化
   - Solution: 高斯椭球是唯一既有 3D 定义、又有 2D 投影闭式解的形状
   - 参数化：3 scale + 4 quaternion (共 7 个值，避免对称矩阵冗余)

3. **$\alpha \in [0,1]$ (不透明度)**
   - Problem: 需要控制"可见性强度"
   - Solution: alpha blending 需要这个标量因子
   - 作用：决定高斯对最终颜色的贡献权重

4. **$c \in \mathbb{R}^3$ (颜色，或 SH 系数)**
   - Axiom: 物体有颜色属性
   - Extension: 用球谐函数 $Y_l^m(\mathbf{d})$ 表达视角相关颜色（view-dependent appearance）

**为什么没有更多？**
- $\boldsymbol{\mu}, \boldsymbol{\Sigma}$ 已经表达了"在哪里、什么形状" ✓
- $\alpha, c$ 已经表达了"可见性、颜色" ✓
- **这是最小完备集**: 缺少任何一个都无法正确渲染

**对比其他表示**:
| 表示 | 参数 | 问题 |
|------|------|------|
| 体素 | RGB + σ (每 voxel) | $V^3$ 内存爆炸 |
| 点云 | $\mathbf{x}, c$ (每 point) | 零维无面积，投影有空洞 |
| NeRF | MLP 权重 (~50MB) | 每帧重算，无法实时 |
| **3DGS** | $\boldsymbol{\mu}, \boldsymbol{\Sigma}, \alpha, c$ | ✅ 最小完备集 ✓ |

**结论**: 这四个元素是"从问题出发"推导出的**必然答案**。
---
