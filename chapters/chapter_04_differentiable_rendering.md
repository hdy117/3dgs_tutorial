...
┌─────────────────────────────────────────────────────────────┐
│                      输入层                                  │
│  N 个高斯 {μ,Σ,α,c} + 相机{K,R,T} + 图像尺寸{H,W}           │
└────────────────────┬────────────────────────────────────────┘
                     ↓
┌─────────────────────────────────────────────────────────────┐
│ 阶段 1: 投影（O(N), 2-5ms）                                  │
│ ─────────────────────────────────────────────────────────── │
│ • μ_cam = R·μ + T               (世界→相机)                 │
│ • μ_2d = K·[X/Z, Y/Z, 1]ᵀ      (透视投影)                   │
│ • Σ_cam = R·Σ·Rᵀ                (协方差旋转)                 │
│ • J = ∂(x_2d)/∂(x_c)            (雅可比矩阵)                 │
│ • Σ_2d = J·Σ_cam·Jᵀ             (误差传播)                   │
│ • depth = Z                      (相机系深度)                │
└────────────────────┬────────────────────────────────────────┘
                     ↓
┌─────────────────────────────────────────────────────────────┐
│ 阶段 2: 深度排序（O(N log N), 5-10ms）                       │
│ ─────────────────────────────────────────────────────────── │
│ sorted_indices = argsort(depth, descending=True)           │
│ 所有参数按 sorted_indices 重排 → 从远到近                    │
└────────────────────┬────────────────────────────────────────┘
                     ↓
┌─────────────────────────────────────────────────────────────┐
│ 阶段 3: Tile 预筛选（O(N), 1-2ms）                           │
│ ─────────────────────────────────────────────────────────── │
│ • radii = 3·√λ_max(Σ_2d)       (3σ包围盒)                   │
│ • bbox = μ_2d ± radii              │
│ • tile_to_gaussians: 倒排索引    ├→ GPU-friendly 连续数组   │
└────────────────────┬────────────────────────────────────────┘
                     ↓
┌─────────────────────────────────────────────────────────────┐
│ 阶段 4: Alpha Blending（O(H×W×k), 5-15ms）                   │
│ ─────────────────────────────────────────────────────────── │
│ for each tile (并行):                                       │
│   C = 0, T_acc = 1.0                                        │
│   for g in tile_gaussians:     # 已按深度排序               │
│     diff = pixel - μ_2d[g]                                  │
│     g_val = α[g]·exp(-½·diffᵀ·Σ_2d⁻¹·diff)                  │
│     C += T_acc · g_val · c[g]     (颜色累积)                │
│     T_acc *= (1 - g_val)          (透射率衰减)              │
│     if T_acc < 0.01: break        # 早停优化               │
└────────────────────┬────────────────────────────────────────┘
                     ↓
┌─────────────────────────────────────────────────────────────┐
│                      输出层                                  │
...
### Q1: "如果 N=3M（三百万高斯），性能会怎么变化？"

<details>
<summary>分析方向</summary>
- 投影：O(N) → 线性增长，~6-15ms
- argsort: O(N log N) → ~log(3)/log(1) ≈ 1.6×，~8-16ms  
- Tile 筛选：O(N) → ~2-4ms（但每个 tile 的高斯数增加）
- Alpha Blending: O(H×W×k)，**k 会增加**！→ 可能从 50ms 飙升到 150ms+

瓶颈在哪里？如何优化？
</details>

### Q2: "训练时深度顺序会变吗？什么时候需要重排？"

<details>
<summary>分析方向</summary>
- 高斯位置 μ在梯度下降中会移动 → depth = Z也会变化
- 但学习率很小（~0.001），单步移动 < 1%
- 每 K 步重排一次：K=多少合适？权衡"顺序稳定性"vs"排序开销"

实际数据：3DGS 论文用 K=1000（每千步重排）
</details>

### Q3: "float16 vs float32 — 哪里可以用半精度？"

<details>
<summary>分析方向</summary>
- 投影计算：Σ_2d = J·Σ·Rᵀ...涉及矩阵乘法，累加误差可能大 → **建议 FP32**
- Alpha Blending: exp(-½Δ²) + 加法累积，FP16 的精度（~3-4 位有效数字）够吗？
- argsort: 比较操作，FP16 完全没问题

实际方案：投影用 FP32，渲染用 FP16 → 速度提升 50%+，质量损失<1PSNR
</details>

### Q4: "如果场景有动态物体（高斯在移动），排序策略需要调整吗？"

<details>
<summary>分析方向</summary>
- 静态场景：每 1000 步重排一次 ✅
- 动态场景：高斯位置可能快速变化 → 深度顺序频繁翻转 → argsort 梯度失效？

解决方案:
a) 更频繁的重排（每 10 步？）→ 性能下降
b) Soft sorting → 完全可微，但慢 2-3×
c) 分层排序：静态高斯 + 动态高斯分开处理

这个问题引出了"动态 3DGS"的研究方向！🔥
</details>
...
✅ **两阶段架构**: 投影 + 全局排序（O(N log N)）→ Tile-based 渲染（O(H×W×k)）  
✅ **Tile Culling**: 16×16 tiles，3σ包围盒，减少**99%+计算量**  
✅ **Alpha Blending**: 从后到前混合 + T_acc<0.01早停  
✅ **可微分性**: argsort 在训练时固定 → "工程可微"而非数学可微  
✅ **性能**: N=1M, H×W=2M → 13-32ms = **30-75 FPS** @ RTX 4090
