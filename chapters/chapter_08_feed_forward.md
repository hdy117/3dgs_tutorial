...
```
符号 | 含义 | 单位 | 典型值（RTX 4090）
-----|------|------|------------------
T_train | 训练每帧时间 | ms | 150-200
T_infer | 推理每帧时间 | ms | 5-20 (目标<10)
FPS | 帧率 | 帧/秒 | 50-100 (目标>60)
N | 高斯数量 | 无 | 10⁵-10⁶
H,W | 图像分辨率 | 像素 | 800×600 - 1920×1080
mem | 内存占用 | MB | 50-150 (float32), 25-75 (float16)
加速比 | T_train/T_infer | × | 20-50×
```
...
符号 | 类型 | 含义 | 阈值 | 说明
-----|------|------|------|------
sorted_indices | (N,) | 深度排序索引 | - | 缓存复用
last_camera | dict | 上次相机位姿 | - | 比较用
trans_thresh | 标量 | 平移阈值 | 0.1m | 超过则重排
rot_thresh | 标量 | 旋转阈值 | 5° | 超过则重排
tile_mapping | list | 高斯→tile映射 | - | 缓存复用
need_resort | 函数 | 是否需要重排 | - | 比较last_camera vs curr
```

### 优化技术效果表

```
技术 | 加速比 | 实现难度 | 稳定性 | 内存影响
-----|--------|----------|--------|----------
关闭autograd | 1.5× | 易 | ✅ 完全稳定 | 无变化
预排序缓存 | 1.2-1.5× | 易 | ⚠️ 相机移动大时失效 | 缓存O(N)
Tile优化 | 5-10× | 中 | ✅ 稳定 | 无变化
float16 | 1.5-2× | 易 | ⚠️ 需测试数值稳定性 | ÷2
融合kernel | 1.1-1.3× | 难 | ⚠️ 调试复杂 | 无变化
总计 | 20-50× | 高 | 需全面测试 | 可能÷2
```

### 内存带宽分析符号

```
硬件 | 带宽 | 数据量(3M高斯) | 理论极限FPS
-----|------|----------------|-------------
RTX 4090 | 1 TB/s | 150 MB | 8e12/150e6 = 53,333
实际瓶颈 | - | - | 50 FPS

瓶颈原因:
- 不是纯带宽: 还有kernel启动、同步、计算开销
- N<1M: 内存带宽瓶颈（global memory读取）
- N>2M: 计算瓶颈（投影+评估）
- 分辨率: 像素数线性增加 → 时间线性增加
```
