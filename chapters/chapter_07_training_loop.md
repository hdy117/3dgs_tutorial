...
符号 | 维度 | 含义 | 更新时机 | 说明
-----|------|------|----------|------
N    | 标量 | 高斯数量 | densify/prune后 | 10⁵-10⁶
μ    | (N,3)| 位置参数 | 每步优化 | 世界坐标
Σ    | (N,3,3)| 协方差参数 | 每步优化 | 对称正定
α    | (N,1)| 不透明度 | 每步优化 | [0,1]
c    | (N,3)| 颜色 | 每步优化 | [0,1]
radii| (N,) | 投影半径（像素）| 每帧渲染 | 用于densify
grads_mu| (N,)| μ梯度范数 | 每N步缓存 | 用于densify
grads_Sigma| (N,)| Σ梯度范数 | 每N步缓存 | 用于densify
optimizer | - | Adam优化器 | 参数变化后重建 | 维护动量、方差状态
```
...
参数组 | 初始LR | 调度点 | LR原因
-------|--------|--------|--------
μ      | 1.6e-4 | 7.5k,15k ×0.1 | 位置需精细调整，梯度量级小
Σ      | 1e-3   | 7.5k,15k ×0.1 | 尺度变化需谨慎，中等LR
α      | 5e-2   | 7.5k,15k ×0.1 | α∈[0,1]范围小，梯度小，需大LR
c      | 5e-3   | 7.5k,15k ×0.1 | 颜色易调，中等LR
```
...
符号 | 计算方式 | 期望 | 诊断
-----|----------|------|------
PSNR | 10·log10(1/L1) | >30(合成)/>25(实景) | 不升: LR↑或初始化差
L_total| 总损失 | 持续下降 | NaN: Σ奇异
N增长| len(gaussians) | 先升后稳 | 不增长: grad_threshold↓
grad_norm| param.grad.norm() | >0 | 接近0: 参数"死"了
```
