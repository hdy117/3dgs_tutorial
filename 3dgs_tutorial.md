...
**核心问题**：微积分、线性代数、概率论——它们不是凭空存在的知识，而是人类为了解决具体问题"逼"出来的工具。

内容：
- **核心理念**：数学不是前置知识，而是解决问题的自然延伸
- **线性代数**：协方差矩阵为什么是 3D 椭球的"母语"（特征分解、旋转的优雅表达）
- **微积分**：梯度为什么是"最优调整方向"（链式法则、反向传播直觉）
...
| 主题 | 教程入口 | 章节数 | 推荐度 |
|------|----------|--------|--------|
| 🔹 **微积分** | [`calculus.md`](./calculus.md) | 10 章 + 习题 | ⭐⭐⭐（训练/优化必读） |
| 🔹 **线性代数** | [`linear_algebra.md`](./linear_algebra.md) | 16 章 + 实验 | ⭐⭐⭐（协方差/投影核心） |

> **使用建议**: 
> - Ch0-2 遇到数学概念看不懂？→ 跳到对应章节查阅
> - 想系统巩固？→ 完整刷一遍 calculus.md + linear_algebra.md（按优先级路径）
