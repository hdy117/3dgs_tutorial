...
假设你在赌场买了一个骰子。老板声称它是公平的（每个面出现概率都是 1/6）。但你决定做个实验来验证——你掷了 **60 次**，记录结果如下：
...
| 3 | 5  | 5/60 = 0.083 |
| 4 | 6  | 6/60 = 0.100 |
| 5 | 20 | 20/60 = 0.333 ← 这个面出现太多了！ |
| 6 | 14 | 14/60 = 0.233 |

**关键问题**：如果骰子真的是公平的，每个面的期望次数应该是 60 × (1/6) = 10 次。但你观察到面 5 出现了 20 次（几乎是期望的两倍）！**这个骰子真的有偏吗？如果有偏，真实的概率分布是什么样的？**

### 🔥 MLE 方法：找到最能让这些观测"自然发生"的参数

假设骰子的真实概率是 θ = [θ₁, θ₂, θ₃, θ₄, θ₅, θ₆]（其中 Σθᵢ = 1）。你的 **60 次掷骰子结果**服从一个多项式分布 Mult(60, θ)。

如果真实的 θ = [1/6, 1/6, 1/6, 1/6, 1/6, 1/6]（公平），那么观测到当前结果的概率为：
$$P(D|\theta_{\text{fair}}) = \frac{60!}{8!7!5!6!20!14!} \times (1/6)^{60} \approx 0.0000048$$

但如果 θ = [1/10, 1/10, 1/10, 1/10, 1/3, 2/15]（面5有偏），概率为：
$$P(D|\theta_{\text{biased}}) = \frac{60!}{8!7!5!6!20!14!} \times (1/10)^8 \times (1/10)^7 \times (1/10)^5 \times (1/10)^6 \times (1/3)^{20} \times (2/15)^{14}$$

**MLE 的目标就是找到使这个概率最大的 θ**！
