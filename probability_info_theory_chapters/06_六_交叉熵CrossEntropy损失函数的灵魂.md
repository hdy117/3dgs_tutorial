...
### 💡 直觉引入：压缩文件时的"用错编码表"代价

想象你在帮一个朋友压缩一份英文文档。你有两种编码方案：

**最优编码（Huffman 编码）** — 根据真实的词频来分配码长：
| 单词 | 真实频率 | 最优码长 (bit) |
|------|---------|---------------|
| the | 7% | ~3.5 bit |
| a | 4% | ~4.6 bit |
| xyzzy | 0.001% | ~13.3 bit |

你用这份"真实频率编码表"压缩文件，平均每个单词需要 **~4.2 bits**。这是最优的 — 不可能更短了（这就是熵 H(P)）。

**不幸的情况**：你误以为文档是中文写的，用了中文词频统计出来的编码表来压缩英文。比如你把 "the"（实际频率7%）当成低频字处理给了一个很长的码（20 bit），而把中文常见字当成高频字给了短码。

用这份**错误的编码表**来压缩真实的英文文档，平均每个单词需要 **~8.5 bits**。比最优方案多花了 4.3 bits！

> 🎯 **这就是交叉熵！** $H(P, Q)$ = 用错误的模型 Q 去编码真实分布 P 的数据时，平均需要的比特数。在这个例子里：
> - H(P) ≈ 4.2 bits（最优，用真实频率编码）
> - H(P, Q) ≈ 8.5 bits（用中文编码表压缩英文）
> **多花的 4.3 bits = 你的错误导致的"信息税"**。
